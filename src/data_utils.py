import pickle
from pathlib import Path
import sentencepiece as spm

from config import (
    TOKENIZER,
    BASE_DIR,
    PREPROCESSED_DIR,
)


# train, valid, test의 corpus와 label을 불러오기
def load_data():
    loaded = []

    for name in ["train", "valid", "test"]:
        load_path = PREPROCESSED_DIR / f"corpus_pad_{name}.pkl"

        with open(load_path, "rb") as f:
            corpus = pickle.load(f)
        
        load_path = PREPROCESSED_DIR / f"label_{name}.pkl"

        with open(load_path, "rb") as f:
            label = pickle.load(f)
        
        loaded.append((corpus, label))

    return loaded


# sentencepiece 불러오기
def load_sentencepiece():
    sp = spm.SentencePieceProcessor()

    sp.load(
        str(BASE_DIR/"checkpoints/tokenizer/sentencepiece.model")
    )

    return sp


# word_to_id 크기 불러오기
def load_vocab_size():
    if TOKENIZER != "subword":
        load_path = PREPROCESSED_DIR / f"word_to_id.pkl"

        with open(load_path, "rb") as f:
            word_to_id = pickle.load(f)
        
        return len(word_to_id)
    else:
        sp = load_sentencepiece()

        return sp.get_piece_size()


# word_to_id, id_to_ word 불러오기
def load_vocab():
    loaded = []

    for name in ["word_to_id", "id_to_word"]:
        load_path = PREPROCESSED_DIR / f"{name}.pkl"

        with open(load_path, "rb") as f:
            loaded.append(pickle.load(f))

    return loaded