...
- 두 모델은 서로 다른 하드웨어에서 학습되어 정량적인 수치로 확인하기 어렵지만 그럼에도 BERT가 GRU보다 계산 비용과 학습 시간이 더 많이 요구된다는 점을 실험을 진행하면서 확인할 수 있었다.
- Transformer는 RNN 계열의 순차 연산 한계를 개선하고 장거리 의존성을 효과적으로 학습하기 위해 설계된 모델이다. 하지만 이번 NSMC 데이터셋은 문장이 비교적 짧고 이진 감성 분류라는 비교적 단순한 과제이므로, RNN 계열의 순차 처리로 인한 계산 비용의 불이익이 크지 않았던 것으로 보인다. 이런 이유로 Transformer의 장점이 충분히 발휘되기 어려웠다.
- 이렇게 높아진 계산 비용에도 불구하고 성능 향상은 Test Accuracy 4.50%p, F1-score 4.57%p 수준에 그쳤다.
- 따라서 NSMC 데이터셋을 대상으로 한 실험에서는 최고 성능만을 고려한다면 BERT가 가장 우수한 선택이지만, 계산 비용과 학습 시간을 함께 고려하면 GRU가 경쟁력 있는 선택이라고 볼 수 있다.